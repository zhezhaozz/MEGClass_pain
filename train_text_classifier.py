...
from preprocessing_utils import load, load_classnames, load_labels, load_text
...
def evaluate(args, model, tokenizer, prefix="", save_res=False):
    eval_task_names = (args.dataset_name,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                probs = np.append(probs, nn.functional.softmax(logits, dim=1).detach().cpu().numpy(), axis=0)
        ...
        # with open(class_probs, "w") as writer:
        #     json.dump(probs.tolist(), writer, indent=2)
...
def main(args):
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, save_res=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="agnews, nyt_coarse, 20news_coarse, dbpedia, yelp_aspect, yelp, yelp_cost",
    )
    parser.add_argument(
        "--model_name_or_path",
    ...
    parser.add_argument("--granularity", type=str, default="document", help="Select either \"sent\" or \"document\".")
    parser.add_argument("--confidence_threshold", default=0.9, help="Training data confidence threshold.")
